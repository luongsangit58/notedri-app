...
**Cập nhật 2026-07-28 (rà soát bug tiềm ẩn toàn bộ Nori Agent, theo yêu cầu user "rà soát lại đảm bảo không có bug nào tiềm ẩn"):**

- **Bug NGHIÊM TRỌNG nhất tìm được - rò rỉ hội thoại chéo tài khoản trên máy dùng chung**: `noriAgentStore.dispose()` đã viết sẵn (xoá `agent`/`uiMessages`/`vehicleContext`...) nhưng **KHÔNG NƠI NÀO TRONG APP GỌI TỚI NÓ** - vì `init()` no-op nếu `agent` đã tồn tại (`if (get().agent) return;`), nghĩa là 1 khi Nori đã khởi tạo cho user A, agent + TOÀN BỘ lịch sử hội thoại (có thể chứa số liệu xe/chi phí nhạy cảm) sống mãi trong bộ nhớ tới khi tắt hẳn app - user B đăng nhập sau trên CÙNG máy (không tắt app) sẽ thấy nguyên lịch sử chat của user A. Đây ĐÚNG lớp lỗi "rò rỉ chéo tài khoản" mà `authStore.logout()` đã chủ động xử lý cho GPS (`clearGpsQueue`) và BLE/OBD (ngắt kết nối + `clearObdSessionQueue`) - nhưng lúc xây Nori Agent không có ai nối `dispose()` vào lượt đăng xuất đó. **Đã sửa**: `authStore.ts`'s `logout()` giờ gọi `useNoriAgentStore.getState().dispose()` (import động, tránh vòng lặp import vì `noriAgentStore.ts` đã import ngược `authStore.ts`), cùng chỗ với BLE disconnect.
- **Bug tiềm ẩn liên quan tìm được lúc sửa lỗi trên**: `agent.onStateChange()`/`agent.onProgress()` (gọi trong `init()`) trả về hàm huỷ đăng ký nhưng TRƯỚC ĐÂY không được lưu lại/gọi ở đâu cả - nếu `dispose()` xảy ra TRONG LÚC 1 lượt hỏi còn đang bay (request mạng chưa xong), rồi `init()` tạo agent MỚI ngay sau đó, request CŨ khi cuối cùng resolve xong (`finally` trong `NoriAgent.sendMessage()`) vẫn gọi listener CŨ - âm thầm ghi đè `isThinking`/`progressStage` của phiên MỚI bằng trạng thái của agent đã bị dispose. **Đã sửa**: lưu 2 hàm huỷ đăng ký (`unsubscribeStateChange`/`unsubscribeProgress`, biến module-scope cùng kiểu với `pendingResolve` đã có sẵn) và gọi trong `dispose()`. Cũng phát hiện `dispose()` cũ thiếu `progressStage: null` trong object reset - đã bổ sung.
- **Bug nhỏ tìm được trong `NoriQuickPopover.tsx`'s hiệu ứng fade mới thêm**: `lastMessage` (dùng cho cả `displayKey` lẫn hiện chữ) lấy nguyên tin nhắn CUỐI CÙNG bất kể vai trò - ngay sau khi user gửi câu hỏi (tin nhắn USER vừa được đẩy vào `uiMessages`) nhưng TRƯỚC KHI `isThinking` kịp bật true, ô giữa có thể thoáng hiện chính câu hỏi của user như thể là câu trả lời của Nori. Nhẹ (thường gộp cùng 1 frame nhờ React 18 tự động batch nhiều `set()` liên tiếp trong cùng 1 tick đồng bộ nên khó thấy bằng mắt), nhưng đã sửa cho đúng-về-logic: chỉ coi `lastMessage` là "câu trả lời để hiện" khi `role==='assistant'`.
- **Verify thêm bug fix**: viết 2 test Jest mới (`noriAgentUnsubscribe.test.ts`) xác nhận listener THỰC SỰ ngừng nhận sự kiện ngay sau khi gọi hàm huỷ đăng ký, kể cả khi request đang bay lúc đó chỉ resolve SAU thời điểm huỷ - dùng `chatFn` giả có thể điều khiển thời điểm resolve thủ công (không dùng `TestHarness.runScenario()` vì cần kiểm soát chính xác lúc nào promise mới resolve, khác kịch bản tiêu thụ mảng theo thứ tự). `npx tsc --noEmit` sạch, Jest **32 suite/272 case pass 100%** (thêm 2 case mới).
- **Đã rà soát nhưng XÁC NHẬN KHÔNG PHẢI bug** (ghi lại để khỏi rà lại lần sau): không có tool nào trùng tên đăng ký (21 tool, `ToolRegistry.register()` sẽ throw nếu trùng); mọi tool trong `LocalIntentMatcher.ts` đều khớp đúng tên tool thật đã đăng ký, không lệch chính tả; mọi rule trong `LocalIntentMatcher.ts` đều có case tương ứng trong `LocalReplyTemplates.ts` (không rơi vào nhánh "chưa biết diễn đạt" oan); `expo-haptics`'s `notificationAsync`/`impactAsync` là `async function` nên MỌI throw (kể cả `UnavailabilityError` lúc module chưa link) đều thành promise reject, `.catch(() => {})` xử lý được hết, không có đường throw đồng bộ nào lọt qua; route backend `cost-summary` không có caller nào khác ngoài Nori Agent dùng tham số `scope`, không xung đột. Case nhỏ CHƯA sửa (chấp nhận được, không phải bug): `describeProgressStage()` phân loại `vehicle.getLifetimeCost` vào nhóm "đang đọc dữ liệu xe" (đúng tiền tố `vehicle.` nhưng về ý nghĩa là dữ liệu CHI PHÍ) - chỉ lệch câu đệm hiển thị, không sai dữ liệu; badge nhấp nháy trên icon nổi ẩn hẳn lúc đã gạt vào cạnh (đã ghi rõ là cố tình ở lượt trước).
