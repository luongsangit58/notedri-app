...
- Gemini (`gemini-2.5-flash`) CHƯA test được thật trong phiên này - `GEMINI_API_KEY` trống ở `.env` local (không phải lỗi, chỉ là chưa cấu hình ở máy dev local để test) - không thể khẳng định chắc chắn hành vi thật, chỉ dựa best-effort: schema function-calling của Gemini generally ổn định hơn Groq về format nhưng CÙNG rủi ro "model nhỏ/free tier không tuân prompt tinh tế 100%".
- **Khuyến nghị**: Phase 1 (hỏi đáp) dùng Groq/Gemini free tier ổn cho test, nhưng vì Phase 2 giờ đã GHI dữ liệu thật (rủi ro cao hơn nếu hiểu sai ý) VÀ 2 provider này CHUNG quota với blog - nên cân nhắc chuyển provider mặc định cho user thật (không phải bản thân dev test) sang `NORI_LLM_PROVIDER=anthropic` (đã có sẵn, chỉ cần đổi biến môi trường + có `ANTHROPIC_API_KEY`), hoặc tối thiểu tách riêng API key Groq/Gemini cho Nori (không dùng chung key với blog) để tránh tranh quota lẫn nhau.
