...
| PremiumScreen | `src/screens/profile/PremiumScreen.tsx` | STACK | No | Premium upgrade screen: Free vs Premium feature comparison, 3/6/12-month subscription packages purchased via real Apple/Google In-App Purchase (RevenueCat SDK, see `src/services/iap/RevenueCatService.ts`), and a Restore Purchases button. Refreshes `authStore.user` after a successful purchase so gates unlock immediately. |
