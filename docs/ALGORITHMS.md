...
**Debounce riêng cho 3 rule điện áp (rà soát 31/7):** `evaluate()` là hàm thuần, chạy trên giá trị RAW từng poll (3s/lần) - 1 mẫu dip tải điện tức thời (quạt làm mát/lốc AC bật, đèn pha) đủ kích rule rồi tắt ngay mẫu kế, dù máy phát KHÔNG có vấn đề thật. Tài liệu ngành (underhoodservice.com - alternator low voltage; diễn đàn kỹ thuật xe) xác nhận dip tạm thời xuống ~13V khi tải tăng đột ngột lúc idle là bình thường - kỹ thuật viên đọc đồng hồ đo liên tục theo thời gian, không kết luận từ 1 lần đọc tức thời. `obdLiveMonitor.debounceVoltageFindings()` chỉ cho 3 rule `charging-voltage-*` qua listener sau khi RAW đã kích liên tiếp ≥2 nhịp poll (~6s) - số nhịp này là lựa chọn kỹ thuật của app (không có con số chuẩn từ tài liệu ngành), không áp dụng cho rule khác vì tín hiệu (nhiệt độ...) có quán tính vật lý riêng và cần báo tức thời.
**Vị trí:** [`src/services/obd/obdLiveMonitor.ts`](../src/services/obd/obdLiveMonitor.ts) (`debounceVoltageFindings`, gọi ngay sau `evaluate()` ở vòng poll live - không áp dụng cho `sessionReport.ts` vì đó đã evaluate trên `voltage_min/voltage_max` tổng hợp cả phiên, không phải mẫu tức thời).
