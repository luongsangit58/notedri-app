  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['bottom']}>
      ...
      <ScrollView
        ...
        {/* Chẩn đoán xe (miễn phí mọi tài khoản) - lựa chọn thứ 2 tách biệt hàng
            OBD Premium ở trên, không icon crown, đi qua OBDSetup với
            purpose='diagnostics' (bỏ qua khoá Premium, đích đến ObdSystemHealth). */}
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('OBDSetup', {
              vehicleId,
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: colors.text, fontWeight: '700', fontSize: 14 }}>{t('vehicle_detail.obd_diagnostics_title')}</Text>
            <Text style={{ color: colors.textSecondary, fontSize: 12, marginTop: 1 }}>
              {t('vehicle_detail.obd_diagnostics_desc')}
            </Text>
          </View>
          <FontAwesome5 name="chevron-right" size={13} color={colors.textSecondary} />
        </TouchableOpacity>
